...
function animatePupil(x, y, duration, easing){
    ...
    let centerDistance = distance(width / 2, height / 2, x, y);
    pupil.animate({
        left: `${newX}px`,
        top: `${newY}px`,
...
function pupilRoam(numLocations, startX, startY){
    if(numLocations > 0){
        ...
        let newX = randomRange(0, width);
        let newY = randomRange(0, height);
        let centerDistance = distance(startX, startY, newX, newY); 
...
function distributeCircles(){
    ...
    if(now - lastDistributionTime > distributionInterval){
        ...
        let angle = 2 * Math.PI / circles.length;
...
function dilate(newRadius, time){
    ...
    let change = dilateStepSize * (newRadius-currentRadius) / time;
